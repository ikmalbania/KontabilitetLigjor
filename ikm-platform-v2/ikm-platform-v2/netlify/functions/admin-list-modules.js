        ...
        course.modules.map(async (m) => {
          ...
          if (contentMeta && !hasTrustworthyMeta) {
            ...
            if (full) {
              ...
              // login.js fix: an un-awaited write risks getting killed
              // when the function's execution environment freezes right
              // after the response is sent, so it might never actually
              // land. Lower stakes here (worst case it just re-tries the
              // backfill on the next load), but still worth doing right.
              await contentStore.setJSON(contentKey, full, { metadata: { blockCount, updatedAt } });
              return {
